...
daily_entries = df[['DATEn','UNIT','ENTRIESn_hourly','weekday']].groupby(['DATEn','UNIT','weekday']).sum()
...
U_daily, p_daily = scipy.stats.mannwhitneyu(no_rain['ENTRIESn_hourly'],rain['ENTRIESn_hourly'])
...
daily_summary = daily_entries[['day', 'ENTRIESn_hourly', 'rain']].groupby('day').sum()
...
with_rain_mean_hourly = np.mean(rain_hourly['ENTRIESn_hourly']) # result = 1743.309
without_rain_mean_hourly = np.mean(no_rain_hourly['ENTRIESn_hourly']) # result = 1896.74

# Mann-Whitney test
# null: rain and no-rain populations have the same probability distribution
# alt: rain and no-rain populations have different distributions
# two-tailed test, p-crit = .05
U_hourly, p_hourly = scipy.stats.mannwhitneyu(no_rain_hourly['ENTRIESn_hourly'],rain_hourly['ENTRIESn_hourly'])
